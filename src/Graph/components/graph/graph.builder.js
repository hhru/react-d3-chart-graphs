...
import { isNil } from 'src/Graph/utils';

import { buildLinkPathDefinition } from 'src/Graph/components/link/link.helper';
import { getMarkerId } from 'src/Graph/components/marker/marker.helper';
import { getNormalizedNodeCoordinates } from 'src/Graph/components/graph/graph.helper';
...
function buildNodeProps(node, config, nodeCallbacks = {}, highlightedNode, highlightedLink, transform) {
    ...
    const labelPosition = node.labelPosition || config.node.labelPosition;
    ...
    let labelClass = config.node.labelClass;
